...
def copiar_criterios_al_test(archivo_origen, archivo_destino):
    try:
        ...
        for i, row in enumerate(hoja_origen.iter_rows(min_row=2, max_row=hoja_origen.max_row, values_only=True), start=7):
            ...
            if row[0] is not None:  # Título no puede ser None
                hoja_destino[f'B{i}'] = row[0]  # Título
                hoja_destino[f'C{i}'] = row[1]  # Contexto
                hoja_destino[f'D{i}'] = row[2]  # Resultado / Comportamiento esperado
                hoja_destino[f'E{i}'] = row[3]  # Detalle Campos
